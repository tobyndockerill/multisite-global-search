 ...
 * Description: Adds the ability to search through blogs into your WordPress 3.0 Multisite installation. Based on my other plugin WPMU GLobal Search.
 * Version: 1.0
...
function ms_global_search_page($atts) {
	...
	if(!empty($term)) {
		...
		"where ".$wheresearch." (post_title LIKE '%%".$term."%%' OR post_content LIKE '%%".$term."%%' OR ".$wpdb->base_prefix."users.display_name LIKE '%%".$term."%%')".
            ...
            foreach($search as $s) {
                ...
                <div <?php post_class('globalsearch_post') ?>>
					...
					<div class="globalsearch_content">
                    	<div class="entry">
                        	<?php echo ms_global_search_get_the_content($s); ?>
                    	</div>
					</div>
                </div>
            <?php
	...
	function ms_global_search_v_query($blogs) {
	    ...
	    foreach ($blogs as $blog) {
	        ...
	        if($blog->blog_id == 1) {
	        	...
	        	$posts_select_query    .= " (SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path, posts{$blog->blog_id}.* FROM {$wpdb->base_prefix}{$blog->blog_id}_posts posts{$blog->blog_id} WHERE posts{$blog->blog_id}.post_type != 'revision' AND posts{$blog->blog_id}.post_status = 'publish') ";
	        	$postmeta_select_query .= " (SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path, postmeta{$blog->blog_id}.* FROM {$wpdb->base_prefix}{$blog->blog_id}_postmeta postmeta{$blog->blog_id}) ";
	        	$comments_select_query .= " (SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path, comments{$blog->blog_id}.* FROM {$wpdb->base_prefix}{$blog->blog_id}_comments comments{$blog->blog_id}) ";
	        }
	        $i++;
	    }
