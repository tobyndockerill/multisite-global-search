...
if ( !function_exists( 'ms_global_search_page' ) ) {
	function ms_global_search_page( $atts ) {
		...
		extract( shortcode_atts( array( 'excerpt' => 'no', 'featured_images' => 'no', 'remove_content_images'=>'no' ), $atts ) );
	                ...
	                <div <?php post_class( 'globalsearch_post' ) ?>>
						...
						<div class="globalsearch_content">
							...
							<div class="entry_thumbnail">
								<?php echo ms_global_search_get_the_post_thumbnail_by_blog( $s->blog_id, $s->ID, "thumbnail" ); ?>
							</div>
							<?php } ?>
	                    	<div class="entry">
	                    		<?php
					if ( strcmp( $excerpt, "yes" ) == 0 ) {
						$excerpt = ms_global_search_get_the_excerpt( $s );
					} else {
						$excerpt = ms_global_search_get_the_content( $s );
					}
...
/* Original concept from http://www.htmlcenter.com/blog/wordpress-multi-site-get-a-featured-image-from-another-blog/ */
if ( !function_exists( 'ms_global_search_get_the_post_thumbnail_by_blog' ) ) {
	function ms_global_search_get_the_post_thumbnail_by_blog( $blog_id=NULL, $post_id=NULL, $size='post-thumbnail', $attrs=NULL ) {
		global $current_blog;
		$sameblog = false;
		...
		if ( !ms_global_search_has_post_thumbnail_by_blog( $blog_id, $post_id ) )
			return false;
		...
		if ( is_multisite() ) { /*This is needed for multisites for the image urls*/
	...
	function ms_global_search_has_post_thumbnail_by_blog( $blog_id=NULL, $post_id=NULL ) {
		if ( empty( $blog_id ) ) {
			global $current_blog;
	...
	function ms_global_search_the_post_thumbnail_by_blog( $blog_id=NULL, $post_id=NULL, $size='post-thumbnail', $attrs=NULL ) {
		echo ms_global_search_get_the_post_thumbnail_by_blog( $blog_id, $post_id, $size, $attrs );
	}
}
